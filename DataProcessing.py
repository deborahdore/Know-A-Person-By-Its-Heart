...
def fill_nan(l):
    for i in range(len(l)):
        if math.isnan(l[i]):
            if i==0: l[i]=l[i+1]/2
            elif i==len(l)-1: l[i]=l[i-1]
            else: l[i]=int((l[i+1]+l[i-1])/2)
    return l[0:-1]
...
def get_features(final_peaks, cleaned_signal):
    PQ_time_list=[]; PT_time_list=[]; QS_time_list=[]; QT_time_list=[]; ST_time_list=[]; PS_time_list=[]
    PQ_ampl_list=[]; QR_ampl_list=[]; RS_ampl_list=[]; ST_ampl_list=[]; QS_ampl_list=[]; PS_ampl_list=[]; PT_ampl_list=[]; QT_ampl_list=[]
    PQ_dist_list=[]; QR_dist_list=[]; RS_dist_list=[]; ST_dist_list=[]; QS_dist_list=[]; PR_dist_list=[]
    PQ_slope_list=[]; QR_slope_list=[]; RS_slope_list=[]; ST_slope_list=[]; QS_slope_list=[]; PT_slope_list=[]; PS_slope_list=[]; QT_slope_list=[]; PR_slope_list=[];
    PQR_angle_list=[]; QRS_angle_list=[]; RST_angle_list=[]; RQS_angle_list=[]; RSQ_angle_list=[]; RTS_angle_list=[];
